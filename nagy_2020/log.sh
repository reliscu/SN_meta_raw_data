...
## Combine fastqs from the same sample and rename files
...
for i in $(seq 2 $n_files); do
  ...
  ## Print all SRA runs associated with a sample:
  ...
  if [[ $n_runs == 2 ]]; then
    cat ${runsR1[@]} > ${ea}/$(echo ${runsR2[1]} | sed s/$sra/$geo/)
    cat ${runsR1[@]} > ${ea}/$(echo ${runsR2[1]} | sed s/$sra/$geo/)
  else 
...
samples=$(awk -v i=$i -F',' 'NR>1{print $1}' ../rundata.csv)
